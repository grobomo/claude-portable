...
When a task is dispatched, the dispatcher may have generated spec-kit artifacts:
- `.specs/<slug>/spec.md` — what to build and success criteria
- `.specs/<slug>/plan.md` — technical implementation plan
- `.specs/<slug>/tasks.md` — ordered task breakdown

**Workflow:**
1. Read `.specs/` FIRST — understand the spec before writing code
2. GSD is enforced: create `.planning/quick/001-<slug>/001-PLAN.md` with Goal + Success Criteria from the spec
3. Only then implement — the GSD gate blocks Bash/Write/Edit/Task until PLAN.md exists
4. Verify every success criterion before creating your PR
5. Write `001-SUMMARY.md` documenting what was done

If no `.specs/` directory exists, create your own PLAN.md from the task description.
