...
def _build_continuous_cmd(request_id: str, worker_ip: str, key_path: str,
    ...
    script_lines = [
        ...
        f"cd {workdir}",
        "",
        "# Pull latest target repo before starting (workers may have stale code)",
        f"git fetch origin {branch} 2>/dev/null || true",
        f"git checkout {branch} 2>/dev/null || true",
        f"git reset --hard origin/{branch} 2>/dev/null || true",
